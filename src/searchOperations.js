...
async function putDocumentInIndex(file, indexName, cookie) {
    try {
        ...
        for (let doc of splitValue) {
            let _id = count;
            let document = JSON.parse(unescape(doc));
        ...
        logger.error("Caught Error fnx-putDocumentInIndex " + err.message);
...
async function queryInsideIndex(query, indexName, cookie) {
    try {
        ...
        let method = "GET";
        let queryOn = "_search";
        // gives all query search result if no query is provided
        let document = query || { query: { match_all: {} } };
